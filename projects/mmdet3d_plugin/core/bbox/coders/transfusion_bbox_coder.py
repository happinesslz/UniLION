...
class TransFusionBBoxCoder(BaseBBoxCoder):
    ...
    def decode(self, heatmap, rot, dim, center, height, vel, iou=None, motion=None, motion_cls=None, iou_rectifier=0, filter=False):
        # class label
        final_preds = heatmap.max(1, keepdims=False).indices
        final_scores = heatmap.max(1, keepdims=False).values
        final_ious = None
        final_motion = None
        final_motion_cls = None

        if iou is not None:
            final_ious = iou.permute(0, 2, 1)[..., 0]

        if motion is not None:
            final_motion = motion
            final_motion_cls = motion_cls
        ...
        if self.post_center_range is not None:
            ...
            for i in range(heatmap.shape[0]):
                ...
                if final_ious is not None:
                    ious = final_ious[i, cmask]
                    predictions_dict['ious'] = ious

                    ious = torch.clamp(ious, min=0, max=1.0)
                    scores = torch.pow(scores, 1 - iou_rectifier) * torch.pow(ious, iou_rectifier)
                    predictions_dict['scores'] = scores

                if final_motion is not None:
                    motion = final_motion[i, cmask]
                    motion_cls = final_motion_cls[i, cmask]
                    trajs = motion.cumsum(dim=-2) + boxes3d[:, None, None, :2]
                    predictions_dict['trajs'] = trajs
                    predictions_dict['trajs_cls'] = motion_cls
