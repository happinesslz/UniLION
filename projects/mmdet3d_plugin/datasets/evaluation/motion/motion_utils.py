        ...
        for sample_annotation_token in sample_annotation_tokens:
            ...
            if box_cls == MotionBox:
                ...
                if fut_traj_local.shape[0] > 0:
                    ...
                    pose_record = nusc.get('ego_pose', sd_rec['ego_pose_token'])
                    info['cam_ego2global_translation'] = pose_record['translation']
                    info['cam_ego2global_rotation'] = pose_record['rotation']
                    ...
                    box.translate(-np.array(info["cam_ego2global_translation"]))
                    box.rotate(Quaternion(info["cam_ego2global_rotation"]).inverse)
...
def accumulate(gt_boxes: EvalBoxes,
    ...
    if len(match_data['min_ade']) == 0:
        return MotionMetricData.no_predictions(), 0
    ...
    for ind in sortind:
        ...
        if is_match:
            taken.add((pred_box.sample_token, match_gt_idx))
            traj_matched += 1
    EPA = (traj_matched - 0.5 * N_fp) / npos  ## same as UniAD
    ...
    return MotionMetricData(recall=rec,
                               ...
                               miss_rate_err=match_data['miss_rate']), EPA
