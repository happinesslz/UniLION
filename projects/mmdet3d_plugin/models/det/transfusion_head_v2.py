...
from mmdet.core import build_bbox_coder, multi_apply, build_assigner, build_sampler
...
class TransFusionHeadV2(TransFusionHead):
    def __init__(
            self,
            num_proposals=128,
            auxiliary=True,
        ...
        super(TransFusionHeadV2, self).__init__(num_proposals, auxiliary, in_channels, hidden_channel, num_classes,
                                                iou_rescore_weight, num_decoder_layers, num_heads, nms_kernel_size,
                                                ffn_channel, dropout, bn_momentum, activation, common_heads,
                                                num_heatmap_convs, bias, loss_cls, loss_iou, loss_bbox, loss_heatmap,
                                                train_cfg, test_cfg, bbox_coder)
        ...
        self.loss_heatmap_ins = build_loss(loss_heatmap)
    ...
    def forward_single(self, inputs):
        ...
        for i in range(self.num_decoder_layers):
            ...
            # Prediction
            res_layer = self.prediction_head[i](query_feat)
