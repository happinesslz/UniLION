...
def create_groundtruth_database(dataset_class_name,
    ...
    dataset_cfg = dict(
        type=dataset_class_name, data_root=data_path, ann_file=info_path, return_gt_info=True, modality=input_modality)
    ...
    if db_save_path is not None:
        data_path = db_save_path
    ...
    for j in track_iter_progress(list(range(len(dataset)))):
        ...
        for i in range(num_obj):
            ...
            img_crop, crop_key, crop_depth = find_img_crop(annos['gt_bboxes_3d'][i].corners.numpy(), input_img,
                                                           input_info, points[point_indices[:, i]])
