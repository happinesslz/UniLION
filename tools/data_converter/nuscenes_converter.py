...
def _fill_trainval_infos(nusc,
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        fce2g_r_mat = Quaternion(
            info['cams']['CAM_FRONT']["ego2global_rotation"]
        ).rotation_matrix
        fce2g_t = np.array(info['cams']['CAM_FRONT']["ego2global_translation"])

        for _cam in info['cams']:
            cam_info = info['cams'][_cam]
            # obtain front camera ego to camera
            ce2g_r_mat = Quaternion(cam_info["ego2global_rotation"]).rotation_matrix
            ce2g_t = np.array(cam_info["ego2global_translation"])
            c2ce_r_mat = Quaternion(cam_info["sensor2ego_rotation"]).rotation_matrix
            c2ce_t = np.array(cam_info["sensor2ego_translation"])

            # fce2c_r = fce2g_r_mat.T @ (np.linalg.inv(ce2g_r_mat).T @ np.linalg.inv(c2ce_r_mat).T)
            # fce2c_t = (fce2g_t @ np.linalg.inv(ce2g_r_mat).T) @ np.linalg.inv(c2ce_r_mat).T
            # fce2c_t -= ce2g_t @ (np.linalg.inv(ce2g_r_mat).T @ np.linalg.inv(c2ce_r_mat).T
            #                      ) + c2ce_t @ np.linalg.inv(c2ce_r_mat).T

            c2fce_r = c2ce_r_mat.T @ ce2g_r_mat.T @ np.linalg.inv(fce2g_r_mat).T
            c2fce_t = (c2ce_t @ ce2g_r_mat.T + ce2g_t) @ np.linalg.inv(fce2g_r_mat).T
            c2fce_t -= fce2g_t @ np.linalg.inv(fce2g_r_mat).T

            # cam_info['camego2cam_rotation'] = fce2c_r.T
            # cam_info['camego2cam_translation'] = fce2c_t

            cam_info['cam2camego_rotation'] = c2fce_r.T
            cam_info['cam2camego_translation'] = c2fce_t

            info['cams'].update({_cam: cam_info})

        # extract map annos in front cam ego
        camego2global = np.eye(4)
        camego2global[:3, :3] = fce2g_r_mat
        camego2global[:3, 3] = fce2g_t

        ce2g_t = list(camego2global[:3, 3])
        ce2g_r = list(Quaternion(matrix=camego2global).q)
        ce2g_r_mat = Quaternion(ce2g_r).rotation_matrix
        # map_geoms = nusc_map_extractor.get_map_geom(map_location, ce2g_t, ce2g_r)
        # map_annos = geom2anno(map_geoms)
        # info['map_annos'] = map_annos

        # obtain sweeps for a single key-frame in lidar
        # all lidar points (key and sweeps) are converted to front camera ego in dataset pipeline
        ...
        # lidar to front camera ego
        l2ce_r = (l2e_r_mat.T @ e2g_r_mat.T) @ np.linalg.inv(ce2g_r_mat).T
        l2ce_t = (l2e_t @ e2g_r_mat.T + e2g_t) @ np.linalg.inv(ce2g_r_mat).T
        l2ce_t -= np.array(ce2g_t) @ np.linalg.inv(ce2g_r_mat).T

        lidar2camego = np.eye(4)
        lidar2camego[:3, :3] = l2ce_r.T
        lidar2camego[3, :3] = l2ce_t

        l2ce_r = Quaternion(matrix=lidar2camego).q
        ce2g_r_mat = Quaternion(ce2g_r).rotation_matrix

        info['lidar2camego_translation'] = list(l2ce_t)
        info['lidar2camego_rotation'] = list(l2ce_r)
        ...
        if not test:
            ...
            # convert boxes from lidar to front cam ego
            for b in boxes:
                b.rotate(Quaternion(info["lidar2ego_rotation"]))
                b.translate(np.array(info["lidar2ego_translation"]))
                b.rotate(Quaternion(info["ego2global_rotation"]))
                b.translate(np.array(info["ego2global_translation"]))
                b.translate(-np.array(info['cams']['CAM_FRONT']["ego2global_translation"]))
                b.rotate(Quaternion(info['cams']['CAM_FRONT']["ego2global_rotation"]).inverse)
            ...
            # convert velo from global to front cam ego
            for i in range(len(boxes)):
                velo = np.array([*velocity[i], 0.0])
                velo = velo @ np.linalg.inv(ce2g_r_mat).T
                velocity[i] = velo[:2]
            ...
            # motion prediction annos: future trajectories offset in cam ego frame and valid mask
            num_box = len(boxes)
            ...
            # motion planning annos: future trajectories offset in front cam ego frame and valid mask
            ...
            # global to front cam ego
            ego_fut_trajs = ego_fut_trajs - np.array(ce2g_t)
            ego_fut_trajs = np.dot(np.linalg.inv(ce2g_r_mat), ego_fut_trajs.T).T
            # drive command according to final fut step offset
            if ego_fut_trajs[-1][1] >= 2:
                command = np.array([1, 0, 0])  # Turn Left
            elif ego_fut_trajs[-1][1] <= -2:
                command = np.array([0, 1, 0])  # Turn Right
            else:
...
def nuscenes_data_prep(root_path,
    ...
    create_groundtruth_database(dataset_name, root_path, info_prefix,
                                f'{out_dir}/{info_prefix}_infos_train.pkl', db_save_path=db_save_path)
...
parser.add_argument(
    ...
    default='./data/kitti',
    required='False',
    help='specify the root path of nuScenes db')
