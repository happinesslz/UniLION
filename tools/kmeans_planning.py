...
fp = 'data/nuscenes/nuscenes_infos_train.pkl'
data = mmcv.load(fp)
...
clusters = []
all_navi_trajs = []
all_navi_trajs.extend(navi_trajs[0])
all_navi_trajs.extend(navi_trajs[1])
all_navi_trajs.extend(navi_trajs[2])
trajs = np.concatenate(all_navi_trajs, axis=0).reshape(-1, 12)
cluster = KMeans(n_clusters=K).fit(trajs).cluster_centers_
cluster = cluster.reshape(-1, 6, 2)
clusters.append(cluster)
for j in range(K):
...
# plt.savefig(f'/data/zliu/vis/motion_planning_{K}', bbox_inches='tight')
# plt.close()

clusters = np.stack(clusters, axis=0)
np.save(f'data/nuscenes/kmeans_planning_{K}.npy', clusters)